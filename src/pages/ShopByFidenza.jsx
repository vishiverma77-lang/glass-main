...
export default function ShopByFidenza() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Fidenza`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY FIDENZA</>}
        description={<> Fidenza stands out for gravel clusters of varying sizes scattered randomly, which reminds of concrete being mixed manually by skilled craftsman, who use their tools to blend it with rocks and gravel of varying coarseness. It's a minimal look but rich in surface and material details, balanced colors and delicate nuances. </>}
        imageSrc="/images/Fidenza.jpeg"
      />
      <ShopFilterBar />
