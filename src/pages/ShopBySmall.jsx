...
export default function ShopBySmall() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Small`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      <ShopHero
        title={<>SHOP THE SMALL FORMATS</>}
        description={<> Porcelain stoneware in classic small size provides a versatile and stylish solution for any setting. Small-format tiles help maximize space while highlighting their decorative power.
        </>}
