...
export default function ShopByPlanks() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Planks`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY <br className="hidden md:block"/> PLANKS</>}
        description={<>Explore our exquisite selection of <span className="text-blue-600 font-medium lowercase">planks</span> formats, where each piece is 
