...
export default function ShopByChevron() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Chevron`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY <br className="hidden md:block"/> CHEVRON</>}
        description={<>Explore our exquisite selection of <span className="text-blue-600 font-medium lowercase">chevron</span> formats, where each piece is 
