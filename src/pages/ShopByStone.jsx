...
export default function ShopByStone() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Stone`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      ...
      <ShopHero
        title={<>Porcelain Stoneware that looks like Stone </>}
        description={<>Structured and natural surfaces with strength and durability of a natural stone, evoke modern surfaces that combine aesthetics and resilience. Elegance and strength in a single porcelatin tile
        </>}
        imageSrc="/images/stone.jpeg"
      />
