...
export default function ShopByChester() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Chester`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY CHESTER</>}
        description={<> The composition echoes the characterstics of the patchwork technique, an innovative intentionally random mix and match, which combines abstract designs in a naturally irregular manner.
In fusion with the actual trends, Chester Collection, evokes picturesque houses full of charms </>}
        imageSrc="/images/Chester.jpeg"
      />
      <ShopFilterBar />
