...
export default function ShopByPage() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Page`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY PAGE</>}
        description={<> Innovative porcelain stoneware collection featuring unique geometric shape, showcasing the long hexagon. This new addition to the market allows architects and designers to explore creative possibilities that go beyond the styles. With stunning finishes that mimic cement, italian marble, and wood, Page bespoke surface offer versatile solutions for modern flooring designs. Elevate your spaces with this fresh approach materials and shapes. </>}
