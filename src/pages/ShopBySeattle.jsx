...
export default function ShopBySeattle() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Seattle`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY SEATTLE</>}
        description={<>Seattle depicts soft, cement and concrete effect porcelain stoneware. Available in extremely contemporary palette of six neutral colors, ranging from sandy beige to darker grey tones. It's inspired by urban design and contemporary architectural trends. The color range allows for the alternating of warm and cool tones, with nuances or accents, to create original, dynamic environments. </>}
