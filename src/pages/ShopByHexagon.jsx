...
export default function ShopByHexagon() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Hexagon`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY <br className="hidden md:block"/> HEXAGON</>}
        description={<>Explore our exquisite selection of <span className="text-blue-600 font-medium lowercase">hexagon</span> formats, where each piece is 
