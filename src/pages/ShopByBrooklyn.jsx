...
export default function ShopByBrooklyn() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Brooklyn`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY BROOKLYN</>}
        description={<> Brooklyn is a design material in porcelain stoneware.The skillfully balanced, subtle veining effects and tone-on-tone streaks give the collection a contemporary character with a warm, inviting touch. 
The collection has been selected to bring a distinctively electic, contemporary design to settings, with an open mix of styles perfect for both commercial and residential projects.  </>}
        imageSrc="/images/Brooklyn.jpeg"
      />
      <ShopFilterBar />
