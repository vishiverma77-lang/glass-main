...
export default function ShopByBrush() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Brush`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY BRUSH</>}
        description={<>The delicate, chiseled lines of the thin porcelain stoneware create a captivating visual texture that instantly draws the eye. The resulting pieces possess an almost ethereal quality, with finely etched ridges and indentations that play with light and shadow to mesmerizing effect. </>}
        imageSrc="/images/Brush.jpeg"
      />
      <ShopFilterBar />
