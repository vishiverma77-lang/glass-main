...
export default function ShopByDenver() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Denver`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY DENVER</>}
        description={<>Denver, is a special project where the natural elements are creatively interpreted according to contemporary design trends. Where everything seems to flow normally but something distracts us from the ordinary.  </>}
        imageSrc="/images/DENVER.jpeg"
      />
      <ShopFilterBar />
