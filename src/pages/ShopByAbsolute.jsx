...
export default function ShopByAbsolute() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Absolute`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      <ShopHero
        title={<>SHOP BY ABSOLUTE</>}
        description={<> Absolute is a design material in porcelain stoneware. The geometric porcelain surface captivate the eye with their striking color palette and mesmerizing abstract design. Crafted from the finest porcelain, this tile exude a sleek, modern elegance that elevates any space.  </>}
        imageSrc="/images/Absolute.jpeg"
      />
      <ShopFilterBar />
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {displayedProducts.map(product => (
              <ProductCard key={product.uniqueKey || product.id} {...product} />
            ))}
          </div>
        )}
