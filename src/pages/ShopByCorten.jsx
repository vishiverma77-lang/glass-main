...
export default function ShopByCorten() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Corten`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY CORTEN</>}
        description={<>Explore our exquisite selection of <span className="text-blue-600 font-medium lowercase">corten</span> collections, where each piece is 
