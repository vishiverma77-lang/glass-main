...
export default function ShopByOxford() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Oxford`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY OXFORD</>}
        description={<>The classic pattern, given a contemporary twist.Details of a culture that evolves into something new, with the allure of traditional majolica tiles providing the inspiration for modern flooring accessible to everyone.The metropolitan look abandons simplicity and takes on a sophisticated elegance.The strong decorative character brings the heritage of the European tradition up to date by proposing a stylistic concept . </>}
