...
export default function ShopByAlpi() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Alpi`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY ALPI</>}
        description={<>A coloured stone in azul and bronze tone, gives a new meaning to the contemporary interiors.With a solid and heavy stone textured look,it's impeccable.The natural finish of the Alpi collection adds a unique texture to your space, and the two-tone colouring provides a subtle contrast that enhances the look.
 </>}
        imageSrc="/images/Alpi.jpeg"
      />
      <ShopFilterBar />
