...
export default function ShopByCerdisa() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products?search=Cerdisa`);
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
            <ShopHero 
        title={<>SHOP BY CERDISA</>}
        description={<> Cerdisa is a design material in porcelain stoneware. A collection with a sophisticated graphic impact in contrasting blue and white, with a distinctive, slightly worn effect that creates a painting allure. Like unrestrained brushstrokes or flowing vibrant liquids, the turbulent colors conjure up the beauty of the cosmos, with magical and seductive imagery. </>}
        imageSrc="/images/Cerdisa.jpeg"
      />
      <ShopFilterBar />
